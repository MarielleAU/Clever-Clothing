...
class AdminDashboard {
  ...
  setupFormListeners() {
    ...
    if (orderUpdateForm) {
      orderUpdateForm.addEventListener("submit", (e) => 
        this.handleOrderUpdateSubmit(e)
      );
    }
  ...
  setupFilterListeners() {
    ...
    if (stockProductId) {
      stockProductId.addEventListener("change", () => this.updateCurrentStock());
    }
  ...
  async viewCustomerVerification(customerId) {
    //To get the database data 
    if (this.usingSupabase) {
      const {data} = await window.sb.from("customers").select("*").eq("id", customerId);
      const object = data[0];
  ...
  async viewCustomer(customerId) {
    ...
    if (this.usingSupabase) {
      ...
      var customerOrders = this.orders.filter(
        (o) => o.customer.email === customer.email
      );
      var totalSpent = customerOrders.reduce(
        (sum, order) => sum + order.total,
        0
      );
    } else {
          ...
          order.status
                    ...
                    <td>
                        <div class="table-actions">
                            ...
                            <button class="table-btn verify" onclick="adminDashboard.viewOrderVerification('${
                              order.id
                            }')">Verify</button>
  ...
  async viewOrder(orderId) {
    //To get the database data
    if (this.usingSupabase) {
      const {data} = await window.sb.from("orders").select("*").eq("id", orderId);
      const object = data[0];
      object.items = JSON.parse(object.items);
      object.date = object.created_at;
            ...
            <div class="order-info">
                ...
                <div class="info-group">
                    ...
                    ${
                      order.customer.govId && order.customer.selfieVerification
                        ? `<button class="secondary-btn" onclick="adminDashboard.viewOrderVerification('${order.id}')" style="margin-top: 10px;">View Documents</button>`
                        : ""
                    }
                </div>
                ...
                <div class="info-group">
                    ...
                    <p><strong>Pre-Invoice:</strong> ${
                      order.hasPreInvoice || order.has_pre_invoice ? "Generated" : "Not generated"
                    }</p>
                    <p><strong>Real Invoice:</strong> ${
                      order.hasRealInvoice || order.has_real_invoice ? "Generated" : "Not generated"
                    }</p>
                </div>
            </div>
    ...
    const updateOrderBtn = document.getElementById("updateOrderStatus");
    updateOrderBtn.onclick = () => this.updateOrderStatus(orderId);
  ...
  async viewOrderVerification(orderId) {
    //To get the database data 
    if (this.usingSupabase) {
      const {data} = await window.sb.from("orders").select("customer_name(*)").eq("id", orderId);
      const object = data[0].customer_name;

      var customer = object;
      if (!customer) return;
    } else {
      const order = this.customers.find((o) => o.id === orderId)
      var customer = order;
      if (!customer) return;
    }
  ...
  async updateOrderStatus(orderId) {
    ...
    document.getElementById("orderUpdateForm").dataset.editId = orderId;

    this.closeModal("orderModal");
    this.showModal("orderUpdateModal");
  }

  async handleOrderUpdateSubmit(e) {
    e.preventDefault();
    ...
    if (this.usingSupabase()) {
      try {
        if (editId) {
          await window.sb.from("orders").update({status: newStatus}).eq("id", editId);
          this.showNotification("Order status updated successfully!", "success");
        
          const order = this.orders.find((o) => o.id === editId);
          if (order) {
        ...
        this.showNotification("Database error while updating order status", "error");
      }
      return;
    }
    ...
    if (this.currentUser.role === "reseller") {
      this.addNotification(
        ...
        `Reseller ${this.currentUser.username} updated order ${editId} to ${newStatus}`,
        "head_admin"
      );
    }
  }
      ...
      .map(
        (invoice) => `
          ...
          invoice.status
                ...
                <td>${new Date(invoice.dateCreated || invoice.created_at).toLocaleDateString()}</td>
                <td>
                    <div class="table-actions">
    ...
    if (this.usingSupabase()) {
      try {
        await window.sb.from("invoices").insert({
          ...
          subtotal: order.total - order.delivery.fee,
        ...
        const {data} = await window.sb
          .from("invoices")
          .select("id")
          .eq("order_id", orderId).eq("type", "pre")
          .order("id", {ascending: false});
        await window.sb
          .from("orders")
          .update({ has_pre_invoice: true , pre_invoice_id: data[0].id})
          .eq("id", orderId);
      } catch (err) {
    ...
    if (this.usingSupabase()) {
      try {
        ...
        const {data} = await window.sb
          .from("invoices")
          .select("id")
          .eq("order_id", orderId).eq("type", "real")
          .order("id", {ascending: false});
        await window.sb
          .from("orders")
          .update({ has_real_invoice: true , real_invoice_id: data[0].id})
          .eq("id", orderId);
      } catch (err) {
  ...
  async viewInvoice(invoiceId) {
    //To get the database data
    if (this.usingSupabase) {
      const {data} = await window.sb.from("invoices").select("*").eq("id", invoiceId);
      const object = data[0];
      object.items = JSON.parse(object.items); 
      object.date = object.created_at;
      ...
      var invoice = this.invoices.find((inv) => inv.id === invoiceId);
      if (!invoice) {
        this.showNotification("Invoice not found", "error");
        return;
      } 
    ...
    <div class="invoice-info">
      <div class="info-group">
        ...
        <p><strong>Order ID:</strong> ${invoice.orderId || invoice.order_id}</p>
        <p><strong>Type:</strong> ${
          invoice.type === "pre" ? "Pre-Invoice" : "Real Invoice"
        }</p>
        <p><strong>Status:</strong> <span class="status-badge ${
        ...
        <p><strong>Date Created:</strong> ${new Date(
          invoice.dateCreated || invoice.created_at
        ).toLocaleDateString()}</p>
      </div>
      
      <div class="info-group">
        <h4>Customer Information</h4>
        <p><strong>Name:</strong> ${invoice.customerName || invoice.customer_name}</p>
        <p><strong>Email:</strong> ${invoice.customerEmail || invoice.customer_email}</p>
        <p><strong>Phone:</strong> ${invoice.customerPhone || invoice.customer_phone}</p>
        <p><strong>Address:</strong> ${invoice.customerAddress || invoice.customer_address}</p>
      </div>
  ...
  `;
    ...
    printInvoiceBtn.onclick = () => this.printInvoice(invoiceId);
  ...
  async showInvoiceVerification(preInvoiceId) {
    //To get the database data
    if (this.usingSupabase) {
      const {data: invoices} = await window.sb.from("invoices").select("*").eq("id", preInvoiceId);
      const object1 = invoices[0];

      var preInvoice = object1;
      if (!preInvoice) return;

      const {data: orders} = await window.sb.from("orders").select("*").eq("id", preInvoice.order_id);
      const object2 = orders[0];

      var order = object2;
      var realInvoice = order.has_real_invoice === true;

    } else {
      var preInvoice = this.invoices.find((i) => i.id === preInvoiceId);
      if (!preInvoice) return;

      var order = this.orders.find((o) => o.id === preInvoice.orderId);
      var realInvoice = this.invoices.find(
    ...
    content.innerHTML = `
            <div class="invoice-comparison">
                <div class="comparison-section">
                    ...
                    <div class="comparison-item">
                        <span>Order ID:</span>
                        <span>${preInvoice.orderId || preInvoice.order_id}</span>
                        <span class="comparison-match">✓</span>
                    </div>
                    <div class="comparison-item">
                        <span>Customer:</span>
                        <span>${preInvoice.customerName || preInvoice.customer_name}</span>
                        <span class="comparison-match">✓</span>
                    </div>
                    <div class="comparison-item">
    ...
    const markInvoiceVerifiedBtn = document.getElementById("markInvoiceVerifiedBtn");
    markInvoiceVerifiedBtn.onclick = () => this.markInvoiceVerified(preInvoiceId);

    this.closeModal("invoiceModal");
    this.showModal("invoiceVerificationModal");
  }

  async markInvoiceVerified(preInvoiceId) {
    // This would be implemented to mark the pre-invoice as verified
    if (this.usingSupabase) {
      await window.sb.from("invoices").update({verified: true}).eq("id", preInvoiceId)
    }else{
      const preInvoice = this.invoices.find((i) => i.id === preInvoiceId);
      preInvoice.verified = true;
    }
  ...
  async editMember(memberId) {
    //To get the database data
    if (this.usingSupabase) {
      const {data} = await window.sb.from("members").select("*").eq("id", memberId);
      const object = data[0];
  ...
  async handleMemberSubmit(e) {
    ...
    if (document.getElementById("memberModalTitle").textContent === "Add Member") {
  ...
  async viewRequest(requestId, type) {
    if (type === "stock") {
      //To get the database data
      if (this.usingSupabase) {
        const {data} = await window.sb.from("stock_requests").select("*").eq("id", requestId);
        const object = data[0];
        
        var request = object;
        if (!request) return; 
      } else {
        var request = this.stockRequests.find((r) => r.id === requestId);
        if (!request) return;  
      }
      ...
      content.innerHTML = `
        <div class="request-info">
          <div class="info-group">
            ...
            <p><strong>Product:</strong> ${request.productName || request.product_name}</p>
            <p><strong>Quantity:</strong> ${request.quantity}</p>
            <p><strong>Requested by:</strong> ${request.requestedBy || request.requested_by}</p>
            <p><strong>Date:</strong> ${new Date(request.date || request.created_at).toLocaleDateString()}</p>
      ...
      //To get the database data
      if (this.usingSupabase) {
        const {data} = await window.sb.from("report_requests").select("*").eq("id", requestId);
        const object = data[0];
      ...
      content.innerHTML = `
        <div class="request-info">
          <div class="info-group">
            <h4>Report Request Information</h4>
            <p><strong>Request ID:</strong> ${request.id}</p>
            <p><strong>Report Type:</strong> ${request.reportType || request.report_type}</p>
            <p><strong>Reseller:</strong> ${request.resellerName || request.reseller_name}</p>
            <p><strong>Date Range:</strong> ${request.dateFrom || request.date_from} to ${request.dateTo || request.date_to}</p>
            <p><strong>Requested by:</strong> ${request.requestedBy || request.requested_by}</p>
            <p><strong>Date:</strong> ${new Date(request.date || request.created_at).toLocaleDateString()}</p>
            <p><strong>Status:</strong> ${request.status}</p>
  ...
  async approveRequest(requestId, type) {
    if (type === "stock") {
      ...
      if (request) {
        ...
        await window.sb.from("stock_requests").update({status: request.status}).eq("id", requestId);
        this.saveStockRequests();

        // Update product stock
        if (this.usingSupabase) {
          const {data} = await window.sb.from("products").select("*").eq("id", request.product_id);
          const object = data[0];
        
          var product = object;
        } else {
          var product = this.products.find((p) => p.id == request.productId);  
        }
        
        if (product) {
          product.stock = (product.stock || 0) + request.quantity;

          await window.sb.from("products").update({stock: product.stock}).eq("id", product.id);
          this.saveProducts();
        }
        ...
        this.addNotification(
          ...
          `Your request for ${request.quantity} units of ${request.productName || request.product_name} has been approved`,
          "reseller",
      ...
      if (request) {
        ...
        await window.sb.from("report_requests").update({status: request.status}).eq("id", requestId);
        this.saveReportRequests();

        // Notify reseller
        this.addNotification(
          "Report Request Approved",
          `Please generate the ${request.reportType || request.report_type} report for ${
          request.dateFrom || request.date_from} to ${request.dateTo || request.date_to}`,
          "reseller",
          request.resellerId || request.reseller_id
        );
      }
    }
  ...
  async rejectRequest(requestId, type) {
    ...
    if (type === "stock") {
      ...
      if (request) {
        ...
        await window.sb.from("stock_requests").update({status: request.status, reason: reason}).eq("id", requestId);
        ...
        this.addNotification(
          ...
          `Your request for ${request.productName || request.product_name} has been rejected. ${
      ...
      if (this.usingSupabase) {
        ...
        var request = this.reportRequests.find((r) => r.id === requestId);
      }

      if (request) {
        request.status = "rejected";

        await window.sb.from("report_requests").update({status: request.status, reason: reason}).eq("id", requestId);
        ...
        this.addNotification(
          ...
          `The ${request.reportType || request.report_type} report request has been rejected. ${
            reason ? "Reason: " + reason : ""
          }`,
          "reseller",
          request.resellerId || request.reseller_id
  ...
  _renderSettings() {
    document.getElementById("storeName").value = 
      this.settings.storeName || this.settings.store_name || "";
    document.getElementById("storeAddress").value =
      this.settings.storeAddress || this.settings.store_address || "";
    document.getElementById("storePhone").value =
      this.settings.storePhone || this.settings.store_phone || "";
    document.getElementById("storeEmail").value =
      this.settings.storeEmail || this.settings.store_email || "";
    document.getElementById("deliveryFee").value =
      this.settings.deliveryFee || this.settings.delivery_fee || 50;
    document.getElementById("resellerCommissionSetting").value =
      this.settings.resellerCommission || this.settings.reseller_commission || 15;
    document.getElementById("autoApproval").value =
      this.settings.autoApproval || this.settings.auto_approval || "false";
  }
    ...
    if (this.usingSupabase()) {
      try {
        ...
        .upsert({
          ...
          store_email: this.settings.storeEmail
        })
      } catch (err) {
  ...
  async handleSystemSettingsSubmit(e) {
    ...
    if (this.usingSupabase()) {
      try {
        ...
        .upsert({
          id: 1, 
          delivery_fee: this.settings.deliveryFee,
          reseller_commission: this.settings.resellerCommission,
          auto_approval: this.settings.autoApproval
        });
      } catch (err) {
        console.error("[Supabase] save settings error", err);
      }
  ...
  async handlePasswordChange(e) {
    ...
    if (memberIndex !== -1) {
      ...
      if (this.usingSupabase) {
        await window.sb.from("members").update({password: newPassword}).eq("id", Number(memberIndex))
      }
        ...
        this.orders = (data || []).map((row) => {
          ...
          return {
            ...
            customer: {
              ...
              altContact: row.customer.altContact || "",
              notes: row.customer.notes || "None",
            },
            items: items,
            delivery: {
              method: row.delivery.method || "delivery",
              location: row.delivery.location || null,
              fee: Number(row.delivery.fee || 0),
              estimatedDate: row.delivery.estimatedDate || "",
            },
            payment: {
              method: row.payment.method || "gcash",
